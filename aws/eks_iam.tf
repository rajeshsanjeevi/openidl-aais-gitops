...
resource "aws_iam_group" "eks_admin_group" {
  name = "${local.std_name}-eks-admin"
  depends_on = [aws_iam_policy.eks_admin_group_assume_policy,
...
resource "aws_iam_policy" "eks_admin_group_assume_policy" {
  ...
  depends_on = [aws_iam_group.eks_admin_group, aws_iam_group_policy_attachment.eks_admin_group_policy_attachment]
}
